...
TFV_traderGetWeaponsMagazinesPrices =
{
	...
	{
		_trader_id = _x;
//		diag_log format["[OGHF] _x:%1 _trader_id:%2",_x,_trader_id];
//		_fish = PVDZE_plr_TradeMenuResult;
//		diag_log format["[OGHF] _fish %1 ",_fish];
//		{ diag_log format["[OGHF] PVDZE_plr_TradeMenuResult Array %1",_x ]; } forEach _fish;
		
	    PVDZE_plr_TradeMenuResult = nil;                     // Hive Traders.
		_fish = [];		
		
//"[OGHF] _typeOfHumanityWeaponsMagazinesPricesRAW [[6739,["HandChemBlue",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"],[6740,["HandChemGreen",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"],[6741,["HandChemRed",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"],[6742,["FlareGreen_M203",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"],[6743,["FlareWhite_M203",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"],[6744,["HandRoadFlare",1],50,[2,"ItemSilverBar",1],[1,"ItemSilverBar",1],0,669,"trade_items"]] "
//"[OGHF] _data [9999,["2000Rnd_762x51_M134",1],99999,[100000,"Coins",1],[50000,"Coins",1],0,478,"trade_items"] "
		
		PVDZE_plr_TradeMenu = [player,_x];

//		diag_log format["[OGHF] PVDZE_plr_TradeMenu %1 ",PVDZE_plr_TradeMenu];
		publicVariableServer "PVDZE_plr_TradeMenu";
//		diag_log format["[OGHF] Pre PVDZE_plr_TradeMenuResult %1 ",PVDZE_plr_TradeMenuResult];
		waitUntil {!isNil "PVDZE_plr_TradeMenuResult"};
//		diag_log format["[OGHF] PVDZE_plr_TradeMenuResult %1 ",PVDZE_plr_TradeMenuResult];
		...
		for "_i" from 0 to ((count _cfgTraderCategory) - 1) do {
			...
			_data = [[9999,[_class,_typeNum],50,_buy,_sell,0,_trader_id,_type]];
			_fish = _fish + _data;
		};	
...
//		diag_log format["[OGHF] _fish %1",_fish];
		...
		if ((count(PVDZE_plr_TradeMenuResult)) > 0) then {
		    _typeOfHumanityWeaponsMagazinesPricesRAW2 = PVDZE_plr_TradeMenuResult;
//			diag_log format["[OGHF] _typeOfHumanityWeaponsMagazinesPricesRAW2 %1 ",_typeOfHumanityWeaponsMagazinesPricesRAW2];
		};

		if ((count(_fish)) > 0) then {
		    _typeOfHumanityWeaponsMagazinesPricesRAW = _fish;
			//diag_log format["[OGHF] _typeOfHumanityWeaponsMagazinesPricesRAW %1 ",_typeOfHumanityWeaponsMagazinesPricesRAW];
		};

		{
//			diag_log format["[OGHF] _x2 %1",_x];
			if (!(((_x select 1) select 0) isKindOf 'AllVehicles') && !(((_x select 1) select 0) in _badItems)) then {
				_physicalPrice = [];
				_intPrice = 0;
				_intPrice = (_x select 4) select 0 / 100;
...
//	diag_log format["[OGHF] _typeOfHumanityWeaponsMagazinesPrices %1 ",_typeOfHumanityWeaponsMagazinesPrices];
...
//		diag_log format["[OGHF] _traderData %1", _traderData];
		_traderDataFirstClean set [(count _traderDataFirstClean),_traderData];
	} forEach serverTraders;
	
//	diag_log format["[OGHF] _traderDataFirstClean %1", _traderDataFirstClean];
...
//	diag_log format["[OGHF] _traderDataFinal %1", _traderDataFinal];
	_traderDataFinal
};
...
TFV_returnChange =
{
    private ["_amount","_return","_complete","_added"];
	_amount = _this select 0;
//	_amount = _amount * 0.6;
	_return = [];
	_complete = false;
//	diag_log format["[OGHF] _amount:%1", _amount]; 
	_added = [player, _amount] call SC_fnc_addCoinsT;
	_added
};

TFV_returnChangeTax1 =
{
    private ["_amount","_return","_complete","_added"];
	_amount = _this select 0;
	_amount = _amount * 0.75;
	_return = [];
	_complete = false;
//	diag_log format["[OGHF] _amount:%1", _amount]; 
	_added = [player, _amount] call SC_fnc_addCoinsT;
...
TFV_returnChangeTax2 =
{
    private ["_amount","_return","_complete","_added"];
	_amount = _this select 0;
	_amount = _amount * 0.85;
	_return = [];
	_complete = false;
//	diag_log format["[OGHF] _amount:%1", _amount]; 
	_added = [player, _amount] call SC_fnc_addCoinsT;
...
//			diag_log format["[OGHF] WEAPONS %1", _cargoTypes];
